...
from django.core.mail import send_mail
from eshop2.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRETS_KEY
import razorpay
...
def shop(Request,mc,sc,br):
    ...
    return render(Request,'shop.html',{'mc':mc,'br':br,'sc':sc,'data':data,'maincategory':maincategory,'subcategory':subcategory,'brand':brand})
...
def updateprofile(Request):
    ...
    if (user.is_superuser):
        ...
        if(Request.method=='POST'):
          ...
          buyer.save()
          return redirect("/profile")
    return render (Request,'updateprofile.html',{'user':buyer})
...
def forgetOTP(Request):
    if (Request.method=='POST'):
        ...
        if(username):
            ...
            messages.error(Request,'Unautorized')
